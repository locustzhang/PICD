...
# ✅ 路径已修改为你当前运行成功的版本
# ==============================================================================
RESULT_DIR = os.environ.get("PICD_RESULT_DIR", "jcp_results_v6_0_4_rawfix")
...
METHOD_LABELS = ["Vanilla", "Penalty", "Fixed-λ", "SA-PINN", "T-Proj"]
OURS = "TwoStage-Proj"
...
def final_metric(data, method, raw_key, proj_key=None, default=0):
    if method == OURS and proj_key and data.get(proj_key) is not None:
        return data.get(proj_key, default)
    return data.get(raw_key, default)
...
def fig1_conservation():
    ...
    if not curves:
        print("  ⚠ 无数据")
        return
    # 重点：bottom 调大，给下方图例留出空间
    fig = plt.figure(figsize=(WIDTH_FULL, 4.5))
    gs = GridSpec(2, 2, figure=fig,
                   hspace=0.35, wspace=0.3,
                   left=0.08, right=0.98,
                   top=0.92, bottom=0.18)  # <-- 改这里：bottom 从0.12→0.18
...
def fig2_pareto_front(table):
    ...
    gs = GridSpec(2,2, figure=fig, hspace=0.35, wspace=0.3,
                   left=0.08, right=0.98, top=0.92, bottom=0.18) # 多留底部空间

    # 先统一准备好 handles/labels（关键：不从单个子图拿）
    from matplotlib.lines import Line2D
    legend_elements = []
    for m in METHOD_KEYS:
        lab = "T-Proj" if m == OURS else METHOD_LABELS[METHOD_KEYS.index(m)]
        mk = "*" if m == OURS else "o"
        sz = 100 if m == OURS else 50
        legend_elements.append(
            Line2D([0], [0], marker=mk, color=COLORS[m], linestyle="",
                   markersize=sz/10, markeredgecolor="black", label=lab)
        )
    ...
    for i,pde in enumerate(PDE_LIST):
        ...
        for m in METHOD_KEYS:
            ...
            try:
                data=table[pde][m]
                x=final_metric(data, m, "mean_l2_raw", "mean_l2_proj")
                y=final_metric(data, m, "mean_constraint_raw", "mean_constraint_proj")
                if x<=0 or y<=0: continue
                marker = "*" if m == OURS else "o"
                s = 100 if m == OURS else 50
                ax.scatter(x, y, s=s, c=COLORS[m], marker=marker,
                           zorder=10 if m == OURS else 3, edgecolors="black", linewidth=0.5)
            except: continue
        ax.set_xscale("log")
        ax.set_yscale("log")
        ax.set_title(PDE_TITLES[i],fontweight="bold")
    ...
    fig.legend(handles=legend_elements,
               loc="lower center",        # 靠下
               bbox_to_anchor=(0.5, 0.04), # y=0.02：最底部
               ncol=5,                    # 5个方法横排
               frameon=True,
               fontsize=7)
...
def fig3_projection_l2(table):
    ...
    if not valid: return
    fig,axes=plt.subplots(1,len(valid),figsize=(WIDTH_FULL,3.0))
    ...
    for i,pde in enumerate(valid):
        ...
        vals=[l2_raw,l2_proj]
        bars=ax.bar([0,1],vals,color=[COLORS[OURS],BASELINE_GRAY],width=0.55,edgecolor="black")
        upper=max(vals)*1.15 if max(vals)>0 else 1.0
        ax.set_ylim(0, upper)
        ...
        for bar, val in zip(bars, vals):
            ax.text(bar.get_x() + bar.get_width()/2, val + upper*0.02, f"{val:.2e}",
...
def fig6_training_loss_kdv(all_losses):
    ...
    fig=plt.figure(figsize=(WIDTH_FULL,4.5))
    # 留出底部空间给图例
    gs=GridSpec(2,2,figure=fig,hspace=0.35,wspace=0.3, left=0.08, right=0.98, top=0.92, bottom=0.18)
    ...
    for i,(k,lab) in enumerate(keys):
        ...
        for m in METHOD_KEYS:
            try:
                ...
                ax.semilogy(y,**style_for_method(m), label=METHOD_LABELS[METHOD_KEYS.index(m)])
            except: continue
        plot_warmup(ax)
    ...
    for i,pde in enumerate(PDE_LIST):
        ...
        for m in METHOD_KEYS:
            try:
                ...
                # 加上 label，才能显示图例
                ax.semilogy(y/y[0],**style_for_method(m), label=METHOD_LABELS[METHOD_KEYS.index(m)])
            except: continue
        plot_warmup(ax)
        ax.set_title(PDE_TITLES[i],fontweight="bold")
...
def analyze_paper_results(table):
    ...
    for pde in PDE_LIST:
        ...
        for m in METHOD_KEYS:
            if m not in table[pde]: continue
            d = table[pde][m]
            l2 = final_metric(d, m, "mean_l2_raw", "mean_l2_proj")
            rl2 = final_metric(d, m, "mean_rel_l2_raw", "mean_rel_l2_proj")
            linf = final_metric(d, m, "mean_linf_raw", "mean_linf_proj")
            cons = final_metric(d, m, "mean_constraint_raw", "mean_constraint_proj")
            res = final_metric(d, m, "mean_residual_raw", "mean_residual_proj")
    ...
    for pde in PDE_LIST:
        ...
        v = table[pde]["Vanilla"]
        o = table[pde][OURS]
        ours_constraint = final_metric(o, OURS, "mean_constraint_raw", "mean_constraint_proj")
        imp_c = (v["mean_constraint_raw"] - ours_constraint) / v["mean_constraint_raw"] * 100
