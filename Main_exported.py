...
CONFIG = ExperimentConfig(
    T_FINAL=1.0,
    NUM_SEEDS=_env_int("PICD_NUM_SEEDS", 5),
...
ITERATIONS = _env_int("PICD_ITERATIONS", 2000)
LR_MILESTONES = [800, 1400, 1800]
LR_GAMMA = 0.3
WARMUP_ITERS = 60

METHOD_FILTER = _env_list("PICD_METHODS")
PDE_FILTER = _env_list("PICD_PDES")
...
def eval_reference_error(model, x_ref, t_ref, u_ref, out_dim, use_proj=False):
    ...
    with torch.no_grad():
        if isinstance(model, TwoStageProjPINN):
            forward_fn = model.forward_proj if use_proj else model.forward_raw
            u_pred = forward_fn(xt).reshape(nt, nx, out_dim).cpu().numpy()
        else:
...
def eval_residual_oos(model, residual_fn, n=N_RESID_TEST, batch_size=500, use_proj=False):
    ...
    with torch.enable_grad():
        while done < n:
            ...
            if isinstance(model, TwoStageProjPINN):
                u = model.forward_proj(xt) if use_proj else model.forward_raw(xt)
            else:
                u = model(xt)
...
def main():
    method_registry = [m for m in METHODS_REGISTRY if METHOD_FILTER is None or m.label in METHOD_FILTER]
    pde_cases = [p for p in PDE_LIST if PDE_FILTER is None or p[0] in PDE_FILTER]

    log.info("=" * 80)
    log.info(f"JCP FINAL v6.0.4 - RAW/PROJ EVALUATION FIX")
    log.info(f"Device: {DEVICE}")
    log.info(f"Result dir: {RESULT_DIR}")
    log.info(f"Methods: {[m.label for m in method_registry]}")
    log.info(f"PDEs: {[p[0] for p in pde_cases]}")
    ...
    for (pde_name, out_dim, pde_type) in pde_cases:
        log.info(f"\n{'='*80}")
        ...
        for MethodCls in method_registry:
